...
// ── Behavioral thresholds (easy to edit) ──
const RECENCY_ACTIVE_DAYS = 60;
const RECENCY_DORMANT_DAYS = 120;
// active < 60d, dormant 60-120d, at-risk > 120d
...
const ReportsPage = () => {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        .from("order_history")
        .select("client_name, price, quantity, order_date");
      setOrders(data ?? []);
      setLoading(false);
    })();
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Analiza RFM i Ranking Klientów</h1>
